...
router.post('/start', upload.single('video'), async (req: any, res: Response): Promise<void> => {
  try {
    ...
    if (modelProvider === 'cloud' && API_KEY && API_KEY !== 'your-api-key') {
      try {
        ...
        console.error('云端API调用失败，切换到本地处理:', apiError)
        taskStore.set(taskId, { 
          ...taskStore.get(taskId)!, 
...
function simulateLocalProcessing(taskId: string, file: any, modelType: string) {
  const modelConfig: Record<string, { steps: number, speed: number, quality: string }> = {
